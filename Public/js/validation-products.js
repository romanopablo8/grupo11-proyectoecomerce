...
const validarCategoria = (input, campo) => {
    const categoria = document.getElementById('category_id')
...
const validarColor = (input, campo) => {
    const categoria = document.getElementById('color_id')
    if (categoria.value == 0 || categoria.value== ""){
...
formulario.addEventListener('submit', (e) => {
    ...
    if (campos.name && campos.descripcion && campos.category_id && campos.color_id && campos.price && campos.discount && campos.descripcionfull ){
        formulario.reset()
    }
})
